...
export default function TrendsPage() {
  ...
  React.useEffect(() => {
    const fetchTrends = async () => {
      try {
        const response = await fetch('https://linkedin-job-api.p.rapidapi.com/job/search?keyword=Software%20developer&page=1', {
          method: 'GET',
          headers: {
            'X-RapidAPI-Key': 'process.env.linkedin-scrapper',
        ...
        const mappedData: TrendData[] = jobData.map((item: any) => ({
          title: item.title,
          growth: `${item.views} views`,
          demand: `${item.applies} applies`, 
          category: 'jobs', 
          icon: IconBriefcase, 
        }));
